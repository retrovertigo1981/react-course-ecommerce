...
function Home() {
  ...
  const renderView = () => {
    if (context.searchByTitle?.length > 0) {
      ...
      return (
        context.filteredItemsByCategory?.map(item => (
          <Card key={item.id} data={item}/>
        ))
      )
    } else {
    ...
    if (!context.filteredItemsByCategory) {
      return (
        <div className="w-80 h-11 relative mb-6">
          <input className=" rounded-lg border border-gray-300 w-full h-full p-2 focus:outline-none"
            type="text" 
            placeholder="Search a Product"
            onChange={(event) => context.handleSearchByTitle(event)}         
            />
            <span className="absolute top-[50%] translate-y-[-50%] right-2"> <MagnifyingGlassIcon className="w-6 h-6 text-gray-400"/></span>
